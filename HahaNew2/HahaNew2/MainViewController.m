...
@synthesize pageTitleStr=pageTitleStr;
@synthesize activity;
@synthesize Team;
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        //self.tabBarItem=[[UITabBarItem alloc]initWithTitle:nil image:[UIImage imageNamed:@"0032.png"] tag:2];
       // self.title=@"最热";//不是这里其作用显示的
...
-(void)notifyLoadFinish
{
    ...
    if (isFristLoad)
    {
        ...
        //进入该tabbar就执行该方法 tableview 显示数据
        isFristLoad = NO;
...
- (void)viewDidLoad
{
    ...
    //self.tabBarItem.title =[pageTitleStr substringFromIndex:4];//under the Press
    
    //self.hahaType = @"hot";
    
    //[self.hahaRpc fetchHahaListInAsyn:self.hahaType pageNum:curPageNumber pageSize:_PAGE_SIZE];
    
    [self.hahaRpc fetchHahaList:self.hahaType pageNum:curPageNumber pageSize:_PAGE_SIZE];
...
{//实现TableView数据源方法
    ...
    if ( item.imgPath == nil && item.imgUrl !=nil)
    {
        ...
        [hahaRpc getHahaItemPicture:item.imgUrl withName:[[item.imgUrl componentsSeparatedByString:@"/"] lastObject] andIndexRow:tepStr];
    ...
    if(item.iconUrl != nil && ![item.iconPath isEqual: @""])
    {
...
{//表示发生了表视图某行被选中的事件，后面的参数具体指明了是哪个表视图（tableView）的哪一行（indexPath里包含）被选中。
    ...
    HahaItemDetailViewController *hahaItemDetailView = [[[HahaItemDetailViewController alloc] initWithNibName:@"HahaItemDetailViewController" bundle:nil]autorelease];
    hahaItemDetailView.hahahItem = itemForDetail;
    // NSLog(@"@@@@@@@@%@",app.hahaItemVC1.hahahItem.name);
...
-(void)publishAHaha//发布一条哈哈
{
    ...
    if (app.isLonIn)
    {
        ...
        PublishOrCommentViewController *publishView = [[[PublishOrCommentViewController alloc] initWithNibName:@"PublishOrCommentViewController" bundle:nil]autorelease] ;
    ...
    {
        LoginViewController *loginView = [[[LoginViewController alloc] initWithNibName:@"LoginViewController" bundle:nil]autorelease] ;
...
-(void)SettingPress
{
    ReturnViewController *returnView = [[ReturnViewController alloc] initWithNibName:@"ReturnViewController" bundle:nil];//一释放就崩溃
