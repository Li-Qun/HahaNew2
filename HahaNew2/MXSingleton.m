...
+(id)sharedInstance
{
    ...
    {
        if ([self class]  == [MXSingleton class]) 
            [NSException raise:NSInternalInconsistencyException format:@"+[NSSingleton sharedInstance] - Abstract class instantiation -"];
        else {
            Singleton *singleton = FindSingleton(self);
            
            if (singleton != NULL) 
                sharedInstance = singleton->_instance;
            else {
                _fromInternalCall = YES;
...
-(void)dealloc
{
    ...
    //
    @synchronized([MXSingleton class]) {
        if (_fromInternalCall == YES) {
            [self singletonDealloc];
            [super dealloc];
        }
        else {
            [NSException raise:NSInternalInconsistencyException format:@"-[NSSingleton dealloc] - invalid call -"];  
        }
    }
}
